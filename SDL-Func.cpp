...
void show(vector<string> page, SDL_Surface *screen, SDL_Surface *red, SDL_Surface *yellow, SDL_Surface *blue, SDL_Surface *green, SDL_Surface *orange, int width, int score, int time, SDL_Rect ScoreBoard, TTF_Font *font, SDL_Color textColor, SDL_Surface *scorepage)
{
	...
	for (int row = 0; row < height; row++)
	{
		for (int column = 0; column < width; column++)
		{
			shape.x = column * 100;
			shape.y = row * 100;
			if (page[row][column] == 'r')
			{
				SDL_BlitSurface(red, NULL, screen, &shape);
			}
			if (page[row][column] == 'b')
			{
				SDL_BlitSurface(blue, NULL, screen, &shape);
			}
			if (page[row][column] == 'y')
			{
				SDL_BlitSurface(yellow, NULL, screen, &shape);
			}
			if (page[row][column] == 'g')
			{
				SDL_BlitSurface(green, NULL, screen, &shape);
			}
			if (page[row][column] == 'o')
			{
				SDL_BlitSurface(orange, NULL, screen, &shape);
			}
		}
	}
